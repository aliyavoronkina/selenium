...
if [ "$cygwin" = "false" ] && [ "$darwin" = "false" ] && [ "$nonstop" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        warn "Could not query maximum file descriptor limit: $MAX_FD_LIMIT, the limit is: $MAX_FD"
    fi
fi
...
if $cygwin || $msys ; then
    ...
    if [ "$OURCYGPATTERN" != "" ] ; then
        list="`echo $list | sed "s|$OURCYGPATTERN||g"`"
    fi
fi

# Split the arguments, and prepend `-classpath` and `-Dorg.gradle.appname=$APP_BASE_NAME` to the Gradle command
params=()
for i in "$@" ; do
    params+=("$i")
done

exec "$JAVACMD" -classpath "$CLASSPATH" -Dorg.gradle.appname="$APP_BASE_NAME" org.gradle.wrapper.GradleWrapperMain "${params[@]}"